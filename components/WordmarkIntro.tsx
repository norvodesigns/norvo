...
export default function WordmarkIntro({ className = "" }: { className?: string }) {
  ...
  return (
    <svg viewBox="367 268 680 251" className={className} style={{ height: "1em", width: "auto" }} role="img" aria-label="Norvo">
      <defs>
        <linearGradient id="wmArc" x1="0" y1="0" x2="1" y2="0">
          <stop offset="0%" stopColor="var(--norvo-teal)" />
          <stop offset="100%" stopColor="var(--norvo-gold)" />
        </linearGradient>
        <clipPath id="wmArchClip" clipPathUnits="userSpaceOnUse">
          <motion.rect
            x={360} y={255} height={275}
            initial={{ width: reduce ? 260 : 0 }}
            animate={{ width: 260 }}
            transition={{ delay: reduce ? 0 : 0.25, duration: reduce ? 0 : 1.05, ease }}
          />
        </clipPath>
        <clipPath id="wmOrvoClip" clipPathUnits="userSpaceOnUse">
          <motion.rect
            x={590} y={255} height={275}
            initial={{ width: reduce ? 470 : 0 }}
            animate={{ width: 470 }}
            transition={{ delay: reduce ? 0 : 1.5, duration: reduce ? 0 : 0.95, ease }}
          />
        </clipPath>
      </defs>

      <motion.g
        initial={{ x: reduce ? 0 : 234 }}
        animate={{ x: 0 }}
        transition={{ delay: reduce ? 0 : 1.45, duration: reduce ? 0 : 0.85, ease: [0.22, 1, 0.36, 1] }}
      >
        <g clipPath="url(#wmArchClip)">
          <g transform="translate(370.6,271.6) scale(0.8) translate(0,305) scale(0.1,-0.1)" fill="url(#wmArc)">
            <path d={ARCH_PATH} />
          </g>
        </g>
        <g clipPath="url(#wmOrvoClip)">
          <g transform="translate(596,354) translate(0,108) scale(0.1,-0.1)" fill="var(--foreground)">
            <path d={ORVO_PATH} />
          </g>
        </g>
      </motion.g>
